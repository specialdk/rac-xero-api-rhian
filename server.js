...
async function initializeDatabase() {
  try {
    ...
    console.log(
      "✅ Database tables and session manager initialized successfully"
    );
  } catch (error) {
...
app.post("/api/session/initialize", async (req, res) => {
  try {
    ...
    console.log(
      `Initializing session ${sessionId} with ${connectedTenantIds.length} companies`
    );
    const startTime = Date.now();

    // Load all company data in parallel (this is the expensive operation)
    const result = await sessionManager.loadSessionData(
      sessionId,
      connectedTenantIds
    );
...
app.post("/api/dashboard/consolidated", async (req, res) => {
  try {
    ...
    if (selectedTenantIds) {
      await sessionManager.setDisplaySelection(
        sessionId,
        selectedTenantIds,
        view
      );
    ...
    const consolidatedData = await sessionManager.getConsolidatedData(
      sessionId,
      selectedTenantIds
    );
...
app.get("/api/session/status/:sessionId", async (req, res) => {
  try {
    ...
    const displaySelection = await sessionManager.getDisplaySelection(
      sessionId
    );
    ...
    res.json({
      ...
      minutesUntilExpiry: sessionStatus.expiresAt
        ? Math.max(
            0,
            Math.floor(
              (new Date(sessionStatus.expiresAt) - new Date()) / (1000 * 60)
            )
          )
        : 0,
    });
...
setInterval(async () => {
  try {
    if (sessionManager) {
      const result = await pool.query(
        "DELETE FROM session_company_data WHERE expires_at < NOW()"
      );
...
app.get("/api/monthly-breakdown/:tenantId", async (req, res) => {
  try {
    ...
    console.log(
      `📅 DEBUG: Starting monthly breakdown for ${tokenData.tenantName}`
    );
    console.log(`📅 DEBUG: Report date: ${reportDate}`);
    ...
    for (const period of monthlyPeriods) {
      try {
        console.log(
          `📊 DEBUG: Fetching P&L for ${period.label} (${period.startDate} to ${period.endDate})`
        );
        ...
        console.error(
          `❌ ERROR loading month ${period.label}:`,
          monthError.message
        );
    ...
    console.log(`📊 DEBUG: Monthly breakdown totals:`);
    console.log(
    ...
    try {
      ...
      if (yoyResponse.ok) {
        ...
        console.log(`🔍 DEBUG: YoY Reconciliation Check:`);
        console.log(
        ...
        console.log(
          `  Profit Reconciled: ${yoyComparison.profitReconciled ? "✅" : "❌"}`
        );
      }
    } catch (yoyError) {
      console.warn(
        `⚠️ Could not fetch YoY data for reconciliation:`,
        yoyError.message
      );
    }
    ...
    console.error("❌ ERROR in monthly breakdown:", error);
    res.status(500).json({
...
app.post("/api/monthly-breakdown", async (req, res) => {
  try {
    ...
    console.error("❌ Monthly breakdown API error:", error);
    res.status(500).json({ error: error.message });
  }
});
